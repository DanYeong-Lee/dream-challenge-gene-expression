...
class DeepFamQ_CRC(nn.Module):
    def __init__(
        ...
        self.lin = nn.Linear(lstm_hidden_dim * 2, lstm_hidden_dim)
    ...
    def forward(self, x):
        ...
        out1 = self.lin(rearrange(x, "L N C -> N L C"))
        out1 = self.fc(rearrange(out1, "N L C -> N (L C)"))
        ...
        out2 = self.fc(rearrange(x, "N C L -> N (L C)"))
