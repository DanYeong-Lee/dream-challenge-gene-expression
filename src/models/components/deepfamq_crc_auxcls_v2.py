...
class DeepFamQ_CRC(nn.Module):
    def __init__(
        ...
        self.fc1 = nn.Sequential(
            nn.Dropout(dropout2),
            nn.Linear(lstm_hidden_dim * 2 * pool_out_len, fc_hidden_dim),
            nn.ReLU(),
        ...
        self.fc2 = nn.Sequential(
            nn.Dropout(dropout2),
            nn.Linear(lstm_hidden_dim * pool_out_len, fc_hidden_dim),
            nn.ReLU(),
            nn.Linear(fc_hidden_dim, fc_hidden_dim),
            nn.ReLU(),
            nn.Linear(fc_hidden_dim, 1)
    ...
    def forward(self, x):
        ...
        out1 = self.fc1(rearrange(x, "L N C -> N (L C)"))
        ...
        out2 = self.fc2(rearrange(x, "N C L -> N (L C)"))
        
        out = out1 * self.alpha + out2 * (1 - self.alpha)
        out = rearrange(out, "N 1 -> N")
