...
class DeepFamQ_MLPDO(nn.Module):
    def __init__(
        self,
        conv_out_dim: int = 320,
        conv_kernel_size: List = [10, 15],
...
class DeepFamQ_GRU(nn.Module):
    def __init__(
        self,
        conv_out_dim: int = 320,
        conv_kernel_size: List = [10, 15],
        pool_size: int = 3,
        gru_hidden_dim: int = 320,
        fc_hidden_dim: int = 64,
        dropout1: float = 0.2,
        dropout2: float = 0.5
    ):
        super().__init__()
        pool_out_len = int(1 + ((110 - pool_size) / pool_size))
        fc_input_dim = gru_hidden_dim * 2 * pool_out_len
        conv_each_dim = int(conv_out_dim / len(conv_kernel_size))
        
        self.conv_blocks = nn.ModuleList([ConvBlock(4, conv_each_dim, k, pool_size, dropout1) for k in conv_kernel_size])
        self.gru = nn.GRU(input_size=conv_out_dim, hidden_size=gru_hidden_dim, bidirectional=True)
...
class DeepFamQ_MHA(nn.Module):
    def __init__(
        ...
        self.conv_blocks = nn.ModuleList([ConvBlock(4, conv_each_dim, k, pool_size, dropout1) for k in conv_kernel_size])
        self.lstm = nn.LSTM(input_size=conv_out_dim, hidden_size=lstm_hidden_dim, bidirectional=True)
        self.q = nn.Linear(lstm_hidden_dim * 2, mha_embed_dim)
        self.k = nn.Linear(lstm_hidden_dim * 2, mha_embed_dim)
        self.v = nn.Linear(lstm_hidden_dim * 2, mha_embed_dim)
...
class DeepFamQ_BN(nn.Module):
    def __init__(
        ...
        self.conv_blocks = nn.ModuleList([ConvBlockBN(4, conv_each_dim, k, pool_size, dropout1) for k in conv_kernel_size])
        self.lstm = nn.LSTM(input_size=conv_out_dim, hidden_size=lstm_hidden_dim, bidirectional=True)
